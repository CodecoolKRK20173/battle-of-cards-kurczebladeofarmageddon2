...
//    public static final String ANSI_WHITE = "\u001B[37m";
    ...
    private void printAllTopCard(ArrayList<Player> players, int selectedAttribute, Player winner) {
        ...
        for (int i = 0; i < maxAttributes; i++) {
            for (Player player : players
                ...
                System.out.print (String.format (i == selectedAttribute ? ("│" + ANSI_RED + " %-28s" + ANSI_RESET + "│") : ("│ %-28s│"), (i + 1) + ". " + player.getTopCard ( ).getAttributesKeyByNumberInString (i) + " = " + player.getTopCard ( ).getAttributesValueArray ( ).get (i)));
            }
            System.out.println ( );
        }
        System.out.println ("└─────────────────────────────┘└─────────────────────────────┘");
    ...
    private int getMaxAttributesOfTopCard(ArrayList<Player> players) {
        ...
        for (Player player : players
            ...
            if (player.getTopCard ( ).getNumberOfAttributes ( ) > i)
                i = player.getTopCard ( ).getNumberOfAttributes ( );
        }
        return i;
    }
    ...
    private void printCardlist(Cardlist list) {
    ...
    private void printSingleCard(Card card) {
        ...
        for (int i = 0; i < numberOfAttributes; i++) {
            System.out.println (String.format ("│ %-28s│", (i + 1) + ". " + keyList.get (i) + " = " + valueList.get (i)));
        }
        System.out.println ("└─────────────────────────────┘");
    ...
    private void printAttributeChoseInputPrompt() {
